...
Cursor-native port of `plugins/agent-plugins/model-builder/agents/model-builder.md`. Cursor subagents have no `tools:` allow-list, so the access below is enforced by convention:

## Access

File read/write/edit, and the `sp-global` (CapIQ) and `daloopa` MCP servers configured in `.cursor/mcp.json`.
...
1. **Pull inputs.** `sp-global` (CapIQ) / `daloopa` MCP for historicals, consensus, and filings.
