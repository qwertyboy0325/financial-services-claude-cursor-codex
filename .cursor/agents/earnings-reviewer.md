...
Cursor-native port of `plugins/agent-plugins/earnings-reviewer/agents/earnings-reviewer.md`. Cursor subagents have no `tools:` allow-list, so the access below is enforced by convention:

## Access

File read/write/edit, and the `factset` and `daloopa` MCP servers configured in `.cursor/mcp.json`.
